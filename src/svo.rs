...
impl Node {
    ...
    fn set(&mut self, is_terminal: bool, is_leaf: bool, children: usize, color: u32, normal: u32) {
...
impl SparseVoxelOctree {
    fn build_octree(
        ...
        #[rustfmt::skip]
        let (cx,cy, cz) = // Offsets of children cubes
        ...
        for i in 0..8 {
            if voxel_grid.sample(cx[i], cy[i], cz[i], half_size) {
                ...
                if half_size != 1 {
                    ...
                    // set child pointer
                    self.node_pool[node_tile_idx + i].set(false, false, child_idx, color, 0);
                } else {
                    self.node_pool[node_tile_idx + i].set(false, true, 0x3fffffff, color, 0);
                }
            }
        }
...
impl From<&VoxelGrid> for SparseVoxelOctree {
    fn from(voxel_grid: &VoxelGrid) -> SparseVoxelOctree {
        ...
        svo.node_pool[0].set(false, false, 1, 0, 0);
